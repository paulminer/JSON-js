...
(function () {
    ...
    function dsonEncodeNumber(n) {
        return String(n).replace('e', 'very').replace('E', 'VERY');
    }
...
// Return the result of stringifying the value.
    ...
    DSONParser.prototype.beginsWith = function(target, advanceIndex) {
        var match = (this.dson.substring(this.index, this.index + target.length) === target);
    ...
    DSONParser.prototype.parseArray = function() {
        ...
        while (this.remaining()) {
            ...
            if (foundFirst) {
                if (!this.beginsWith('and', true) && !this.beginsWith('also', true)) {
    ...
    DSONParser.prototype.parseString = function() {
        ...
        if (!this.beginsWith('"', true)) {
            throw new SyntaxError('DSON.parse: Expected double-quote at start of string');
        }
        ...
        while (this.remaining()) {
            ...
            if (c == '\\') {
                ...
                if (ind >= 0) {
                    ...
                    if (this.remaining() < 6) {
                        throw new SyntaxError('DSON.parse: Invalid unicode escape sequence in string');
                    }
                    var u = 0;
                    for (var i = 0; i < 6; i++) {
                    ...
                    if (u >= 65536) {
                        throw new SyntaxError('DSON.parse: Invalid unicode escape sequence in string (too large)');
                    }
                    
                    s += String.fromCharCode(u);
                }
                else {
                    throw new SyntaxError('DSON.parse: Invalid escape sequence in string');
                }
            }
            else {
                s += c;
